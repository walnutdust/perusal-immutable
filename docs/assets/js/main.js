function arrayToNest(array) {
  for (var _a, _b, res = {}, i = array.length - 1; i >= 0; i--)
    i == array.length - 1
      ? ((_a = {}), (_a[array[i]] = array[i]), (res = _a))
      : ((_b = {}), (_b[array[i]] = res), (res = _b));
  return res;
}
  ...
  for (var p in obj2)
    try {
      obj2[p].constructor == Object
        ? (obj1[p].constructor !== Object && (obj1[p] = {Overview: obj1[p]}),
...
function renderSimpleHTMLRecursive(obj, package, spacing) {
  void 0 === package && (package = ''), void 0 === spacing && (spacing = '&emsp;');
  for (var html = '', shownPackages = [], _i = 0, _a = Object.keys(obj); _i < _a.length; _i++) {
    var key = _a[_i];
    if ('object' == typeof obj[key])
      html += renderHTMLRecursive(obj[key], package + '_' + key, spacing + '&emsp;');
    else {
      -1 === shownPackages.indexOf(package) &&
        ((html += '<div>' + package.replace(/_/g, '/') + '</div>'), shownPackages.push(package));
      var href = '';
      -1 == window.location.href.indexOf('modules') && (href = 'modules/'),
        ...
        (href += package
          ? 'Overview' === key
            ? package.substr(1) + '.html'
            : package.substr(1) + '_' + sanitizeModuleName(key) + '.html'
          : sanitizeModuleName(key) + '.html'),
...
!(function(typedoc) {
  ...
  window.addEventListener('load', function() {
    if (document.querySelector('.tsd-navigation.outline')) {
      ...
      var listItems = document.querySelectorAll('.tsd-navigation .tsd-kind-external-module');
      Array.from(listItems).forEach(function(el) {
        el.remove();
      });
      document.querySelector('.tsd-navigation ul').innerHTML += renderSimpleHTMLRecursive(
        hierarchy_1
      );
    }
  });
  ...
  !(function(search) {
    function createIndex() {
      ...
      var rows = search.data.rows,
        pos = 0,
        length = rows.length;
      batch(), initializePriorityResults();
    }
    function initializePriorityResults() {
      ...
      this.priorityResults = priorityResultsDom.map(function(node) {
        var element = node;
        return {name: element.dataset.name, subtitle: element.dataset.subtitle};
      });
    }
    function loadIndex() {
    ...
    function updateResults() {
      ...
      if (loadingState == SearchLoadingState.Ready) {
        ...
        for (var res = index.search(query), i = 0, c = Math.min(10, res.length); i < c; i++) {
          ...
          for (var priority = !1, _i = 0, _a = this.priorityResults; _i < _a.length; _i++) {
            var result = _a[_i];
